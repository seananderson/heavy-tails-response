...
This runs the models with a random walk and fixed observation error:
...
pdf("fig.pdf", width = 6, height = 5)
layout(rbind(c(1, 4), c(2, 4), c(3, 4)))
par(mar = c(0,0,0,0.5), oma = c(3, 3.0, 1, 12), tck = -0.015, mgp = c(2, 0.5, 0), col.axis = "grey25", col = "grey25")
par(cex = 0.8)
# plot(1, 1, type = "n", xlim = c(1, 4), ylim = c(1, 5), xlab = "", ylab = "", yaxt = "n", xaxs = "i", yaxs = "i", las = 1)
par(xpd = FALSE)
...
bean <- function(x) {
  if (ii == 3) scale_ <- 0.8
  plot(1, 1, type = "n", xlim = c(0.4, 4.2), ylim = c(0.8, 4.1), xlab = "", ylab = "", yaxt = "n", xaxs = "i", yaxs = "i", las = 1,
    xaxt = "n", ann = FALSE, bty = "n")
  for (i in unique(x$x)) {
    xx <- filter(x, x == i)
    polygon(c(xx$x - xx$violinwidth*scale_, rev(xx$x + xx$violinwidth*scale_)), c(xx$y, rev(xx$y)),
  ...
  abline(h = 1, col = "grey70", lty = 2, lwd = 1.2)
}
plyr::d_ply(d, "PANEL", function(x) bean(x))

par(xpd = NA)
axis(1, col = "grey", at = 1:4, labels = unique(obs_sim$obs_sd))
mtext("Observation CV", side = 1, line = 1.7)
mtext("Maximum observed r / r", side = 2, line = 1.5, outer = TRUE)
par(xpd = FALSE)

rm <- rmax_obs
rm$id <- sub("Cabbage root fly or maggot", "Cabbage root fly", rm$id)
rm$id <- sub("Pine looper or Bordered white", "Pine looper", rm$id)
rm$id <- gsub("  ", " ", rm$id)
rm$id <- gsub(" / ", ", ", rm$id)
rm <- arrange(rm, dif)
plot(1, 1, xlim = c(0, 7), ylim = c(1, nrow(rm)), type = "n",
  ann = FALSE, ylab = "", xlab = "", bty = "n", xaxt = "n", yaxt = "n")
points(rm$`50%`, 1:nrow(rm), pch = 19, col = "grey30")
segments(rm$`2.5%`, 1:nrow(rm), rm$`97.5`, 1:nrow(rm), col = "grey30")
points(rm$r_max, 1:nrow(rm), col = "grey30", bg = "#FFFFFF", pch = 21)
points(rm$Rho, 1:nrow(rm), pch = 4, col = "red")
box(col = "grey50")
axis(1, col = "grey50", at = seq(0, 8, 2))
axis(2, side = 4, at = 1:nrow(rm), labels = rm$id, las = 1, tcl = 0, col = "grey50")
add_label("D", xfrac = 0.82, yfrac = 0.035, cex = 1.2)
par(xpd = NA)
mtext("Maximum r", side = 1, line = 1.7)
dev.off()
```
